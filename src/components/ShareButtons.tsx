...
export default function ShareButtons({ title }: Props) {
  ...
  return (
    <div className="mt-12 pt-8 border-t border-[var(--border)]">
      <p className="text-sm text-[var(--muted)] mb-4">Podeli ovaj post</p>
      <div className="flex gap-3">
        {links.map(({ label, href, icon }) => (
          <a
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[var(--border)] text-sm text-[var(--muted)] hover:text-[var(--foreground)] hover:border-[var(--foreground)] transition-colors"
          >
            {icon}
            <span>{label}</span>
          </a>
